...
import { changeMemberRoleService, createWorkspaceService, deleteWorkspaceService, getAllWorkspaceIsUserMemberService, getWorkspaceAnalyticsService, getWorkspaceByIdService, getWorkspaceMembersService, updateWorkspaceByIdService} from "../services/workspace.service.js";
import { roleGuard } from "../utils/roleGuard.js";
import { changeRoleSchema, createWorkspaceSchema, workspaceIdSchema } from "../validation/workspace.validation.js";
...
export const getAllWorkspaceUserIsMemberController = asyncHandler(async(req,res)=>{
    const userId = req.user?._id;
...
export const getWorkspaceMemberController = asyncHandler(async(req,res)=>{
    const workspaceId = workspaceIdSchema.parse(req.params.id);
    const userId=req.user?._id;
    const {role}=await getMemberRoleInWohrkspace(userId , workspaceId);
    roleGuard(role,[Permissions.VIEW_ONLY]);
    const {members , roles}=await getWorkspaceMembersService(workspaceId);
    return res.status(HTTPSTATUS.OK).json({message:"workspace members retrived successfully ",members ,roles});
...
export const changeWorkspaceMemberRoleController = asyncHandler(async(req,res)=>{
    const workspaceId = workspaceIdSchema.parse(req.params.id);
    const {memberId , roleId}=await changeRoleSchema.parse(req.body);
    const userId = req.user?._id;
    const {role} = await getMemberRoleInWorkspace(userId , workspaceId);
...
export const updateWorkspaceByIdController = asyncHandler(async(req,res)=>{
    const body = updateWorkspaceIdSchema.parse(req.params.id);
    ...
    return res.staus(HTTPSTATUS.OK).json({
